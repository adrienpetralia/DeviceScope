...
class ResNet5LN(nn.Module):
	def __init__(self, in_channels=1, mid_channels=64, kernel_sizes=[7, 5, 3], nb_class=2):
		...
		self.layers = nn.Sequential(*[
			...
			ResNetBlock(in_channels=mid_channels, out_channels=mid_channels, kernel_sizes=kernel_sizes),
			ResNetBlock(in_channels=mid_channels, out_channels=mid_channels * 2, kernel_sizes=kernel_sizes),
			ResNetBlock(in_channels=mid_channels * 2, out_channels=mid_channels * 4, kernel_sizes=kernel_sizes),
			ResNetBlock(in_channels=mid_channels * 4, out_channels=mid_channels * 4, kernel_sizes=kernel_sizes),
		])
		self.linear = nn.Linear(mid_channels * 4, nb_class)
